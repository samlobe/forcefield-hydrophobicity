...
def potential_energy(seed_scale, solvent_scale):
...
for frame in tqdm(np.arange(0, n_frames, 10)):
    ...
    seed_potential = potential_energy(1, 0)
    solvent_potential = potential_energy(0, 1)
    ...
    interaction_energy = total_potential - seed_potential - solvent_potential
    interaction_energies.append(interaction_energy.value_in_unit(kilojoules_per_mole))
    # print(f"Coulombic potential energy:\n{coul_energy.value_in_unit(kilojoules_per_mole):.0f} kJ/mol")
    
    # CHARMM36m does not have a custom nonbonded force. LJ potential is calculated above
...
energies = pd.DataFrame({'potential': interaction_energies})
...
plt.hist(interaction_energies, bins=100)
plt.xlabel('Potential Energy (kJ/mol)')
plt.ylabel('Frequency')
plt.title('Histogram of Seed-Water Potential Energy')
plt.show()
#%%